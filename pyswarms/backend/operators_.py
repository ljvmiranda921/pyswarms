...
def update_pbest(swarm):
    ...
    try:
        ...
        mask_cost = (swarm.cost < swarm.pbest_cost)
        mask_pos = np.repeat(mask_cost[:, np.newaxis], swarm.dimensions, axis=1)
        # Apply masks
        new_pbest_pos = np.where(~mask_pos, swarm.pbest_pos, swarm.positions)
        new_pbest_cost = np.where(~mask_cost, swarm.pbest_cost, swarm.cost)
